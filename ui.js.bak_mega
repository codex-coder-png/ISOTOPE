...
(function () {
  ...
  function selectVault(id) {
    ...
    $('#vd-desc').textContent = own ? (el.mol ? el.desc : CATS[el.cat].blurb) : 'CLASSIFIED — UNLOCK THIS ELEMENT TO REVEAL ITS PROPERTIES.';
    ...
    const act = el.act;
    const sigs = el.signatures || [];
    const selectedSig = own ? (SAVE.getSignature ? SAVE.getSignature(id) : 0) : 0;
    $('#vd-active').innerHTML = own
      ? `<div class="main-ability"><b style="color:${col}">MAIN ABILITY — ${act ? act.ic+' '+act.name : 'COMPOUND REACTION'}</b><br>${act ? act.desc : (el.desc||'Compound reaction traits.')}</div>
         <div class="sig-choice-title">SIGNATURE [Q] — CHOOSE 1 OF 2</div>
         <div class="sig-choice-grid">${sigs.map((s,i)=>`<button class="sig-choice ${i===selectedSig?'selected':''}" data-sig="${i}"><strong>${s.ic} ${s.name}</strong><span>${s.desc}</span>${i===selectedSig?'<em>SELECTED</em>':''}</button>`).join('')}</div>`
      : `<b style="color:${col}">MAIN ABILITY — ???</b><br>Ability details and both signature choices are hidden until this ${el.mol?'compound':'element'} is unlocked.`;
    if(own) document.querySelectorAll('.sig-choice').forEach(btn=>btn.onclick=()=>{
      const slot=Number(btn.dataset.sig); if(SAVE.setSignature) SAVE.setSignature(id,slot);
    ...
    else if (own) { b.textContent = 'SELECT'; b.disabled = false; b.onclick = () => { SAVE.sel = id; SAVE.save(); if (window.NET && NET.setMyElement) NET.setMyElement(id); SFX.unlock(); renderVault(); toast(el.name + ' deployed', 'good') } }
    else {
      b.textContent = 'UNLOCK · ◈ ' + el.cost; b.disabled = false; b.onclick = () => {
        if (DATA.isOwned(id)) { renderVault(); return; }
        if (SAVE.unlockElement(id, el.cost)) { SFX.unlock(); toast(el.name + ' unlocked!', 'gold'); renderVault() }
  ...
  function renderLab() {
    ...
    [...new Set(SAVE.raw.unlocked)].map(id => ELEMS[id]).filter(Boolean).sort((a, b) => a.n - b.n).forEach(e => add(e.sym, e.sym, e.hue, e.n));
...
/* 
/* ISO_ABILITY_3CHOICE_UI */
(function(){
if(window.__ISO3UI__) return; window.__ISO3UI__=true;
if(window.SAVE){
  var origSet=SAVE.setSignature;
  SAVE.getSignature=function(id){ try{id=DATA.canonicalId(id);}catch(e){} var v=0; try{ var st=JSON.parse(localStorage.getItem('iso_signature_slot_3choice')||'{}'); if(st[id]!=null) v=Number(st[id])||0; }catch(e){} return Math.max(0,Math.min(2,v)); };
  SAVE.setSignature=function(id,slot){ try{id=DATA.canonicalId(id);}catch(e){} slot=Math.max(0,Math.min(2,Number(slot)||0)); var st={}; try{ st=JSON.parse(localStorage.getItem('iso_signature_slot_3choice')||'{}'); }catch(e){} st[id]=slot; try{ localStorage.setItem('iso_signature_slot_3choice',JSON.stringify(st)); }catch(e){} if(origSet){ try{origSet.call(SAVE,id,slot);}catch(e){} } if(SAVE.save) SAVE.save(); };
}
function patchPanel(id){
 try{ id=DATA.canonicalId(id); }catch(e){}
 var el=EL(id), own=DATA.isOwned(id), box=document.getElementById('vd-active');
 if(!box) return;
 var col='hsl('+el.hue+' 72% 62%)';
 if(!own){ box.innerHTML='<b style="color:'+col+'">ABILITY OPTIONS — ???</b><br>Three selectable abilities are hidden until this '+(el.mol?'compound':'element')+' is unlocked.'; return; }
 var choices=(el.choices&&el.choices.length)?el.choices:(el.signatures||[]); choices=choices.slice(0,3);
 var sel=SAVE.getSignature?SAVE.getSignature(id):0;
 var html='<div class="sig-choice-title" style="color:'+col+'">SELECTABLE ABILITIES — CHOOSE 1 OF 3</div><div class="sig-choice-grid">';
 choices.forEach(function(c,i){
  html+='<button class="sig-choice '+(i===sel?'selected':'')+'" data-sig="'+i+'"><strong>'+(c.ic||'✦')+' '+(c.name||('Ability '+(i+1)))+'</strong><span>'+(c.desc||'Unique ability.')+'</span>'+(i===sel?'<em>SELECTED</em>':'')+'</button>';
 });
 html+='</div>';
 box.innerHTML=html;
 Array.prototype.forEach.call(box.querySelectorAll('.sig-choice'),function(btn){
  btn.onclick=function(){
   var slot=Number(btn.dataset.sig);
   if(SAVE.setSignature) SAVE.setSignature(id,slot);
   if(window.NET&&NET.setMySignature) NET.setMySignature(slot);
   SFX.unlock(); selectVault(id); toast('ABILITY '+(slot+1)+' EQUIPPED','good');
  };
 });
}
var oldSelect=selectVault;
selectVault=function(id){ oldSelect(id); patchPanel(id); };
try{ if(vdTarget) selectVault(vdTarget); else if(SAVE.sel) selectVault(SAVE.sel); }catch(e){}
})();
})();