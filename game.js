...
(function () {
  ...
  function openLevel() { RUN.state = 'level'; fillCards(); document.getElementById('m-level').classList.remove('hidden'); broadcastGameState(true); }
  ...
  function broadcastGameState(force) {
    ...
    if (!force && RUN.t - RUN.lastSnapshotAt < 1 / 20) return;
  ...
  if (window.NET) {
    ...
    NET.onStateSnapshot = (snapshot) => {
      ...
      if (RUN.state === 'level') {
        ...
        if (el('cards')) {
          el('cards').innerHTML = `
        <div class="panel" style="padding:20px;font-family:var(--mono);color:var(--tx2)">
          HOST IS SELECTING MODULES…
        </div>
      `;
        }
      } else {
      ...
      if (snapshot.players) {
        snapshot.players.forEach(sp => {
          ...
          if (first || d2(p.x, p.y, tx, ty) > 220 * 220) {
            p.x = tx;
            p.y = ty;
          } else {
            p.x += (tx - p.x) * 0.4;
          ...
          p.activeCd = sp.activeCd || 0;
          p.iframes = sp.iframes || 0;
        });
      }
      ...
      if (snapshot.enemies) {
        ...
        RUN.enemies = snapshot.enemies.map((se, i) => {
          ...
          const old = RUN.enemyMap.get(id);

          let nx = tx;
          let ny = ty;

          if (old && !first && d2(old.x, old.y, tx, ty) < 240 * 240) {
            nx = old.x + (tx - old.x) * 0.5;
            ny = old.y + (ty - old.y) * 0.5;
          }

          newMap.set(id, { x: nx, y: ny });
      ...
      if (snapshot.bullets) {
        RUN.bullets = snapshot.bullets.map(b => ({
          ...b,
          x: b.x * sx,
          y: b.y * sy,
          r: Math.max(2, b.r * s)
      ...
      if (snapshot.ebullets) {
        RUN.ebullets = snapshot.ebullets.map(b => ({
          ...b,
          x: b.x * sx,
          y: b.y * sy,
          r: Math.max(2, b.r * s)
      ...
      if (snapshot.pickups) {
        RUN.pickups = snapshot.pickups.map(k => ({
          ...k,
          x: k.x * sx,
          y: k.y * sy,
          vx: 0,
